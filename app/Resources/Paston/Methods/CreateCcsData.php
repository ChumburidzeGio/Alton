...
class CreateCcsData extends AbstractMethodRequest
{
    ...
    protected $rollsTransmissionOldToName = [
        0 => null,
        1 => 'M',
        2 => 'A',
    ];

    protected $rollsCoachworkToBodyType = [
        0 => null, // 'Alle',
        10 => 'BUS', //'Bestel ',
    ...
    public function executeFunction()
    {
        ...
        // Mileage not known, try to calculate it
        if (!isset($this->params[ResourceInterface::USED_MILEAGE]) && isset($this->params[ResourceInterface::USER], $this->params[ResourceInterface::WEBSITE])) {
            ...
            $resource = Resource::where(['name' => 'product.carinsurance'])->firstOrFail();
            try {
                $productParams = ['enabled' => 1, 'active' => 1, '_offset' => 0, '_limit' => 999999, '__id' => $this->params[ResourceInterface::PRODUCT_ID]] + $this->params;
                $products = ResourceHelper::call($resource, 'index', $productParams);

                if (count($products) != 1)
                    Log::error('CCS mileage request failed - too many or too few results ('. count($products) .'). Input: '. json_encode($productParams) ."\nOutput: ". json_encode($products));
                else if (isset($products[0][ResourceInterface::USED_MILEAGE]))
                    $this->params[ResourceInterface::USED_MILEAGE] = $products[0][ResourceInterface::USED_MILEAGE];

            } catch (\Exception $e) {
                // Ignore errors, but do report them in the logs
                Log::error('CCS mileage request failed: '. $e->getMessage() .' - Input: '. json_encode($productParams));
            }
        }
        ...
        $this->result = [
            ...
            ResourceInterface::URL => ((app()->configure('resource_paston')) ? '' : config('resource_paston.settings.meeus_ccs_form_url')),
