...
class AutoPremieBerekenClient extends RollsAbstractSoapRequest
{
    ...
    protected $arguments = [
        ...
        ResourceInterface::GET_ADDITIONAL_COVERAGES => [
            'rules' => 'bool',
        ],
    ];
    ...
    public function getResult()
    {
        ...
        if($this->licenseplate){
            $carResult = $this->getCarResult($this->getCarDetails(), 'car_option_list');
        ...
        return $this->getMotorizedPremieResult('car_option_list', $this->extended, $advice);
    }
    ...
    public function setParams(Array $params)
    {
        ...
        /**
         * Only do an extended request if `get_additional_coverages` is set, or if not set, look if there is only one product.
         */
        $this->extended = isset($params[ResourceInterface::GET_ADDITIONAL_COVERAGES]) ? (bool)$params[ResourceInterface::GET_ADDITIONAL_COVERAGES] : (count($idsArray) == 1);
        cw('Extended is ' . $this->extended);

        $this->init($this->extended ? ((app()->configure('resource_rolls')) ? '' : config('resource_rolls.functions.aanvullendepremie_auto_function')) : ((app()->configure('resource_rolls')) ? '' : config('resource_rolls.functions.premie_auto_function')));
     ...
     * @return array
     */
    protected function getCarResult($result, $optionList = 'car_option_list')
    {
        ...
        $return[ResourceInterface::WEIGHT]              = $result->Gewicht . "";
        $return[ResourceInterface::LICENSEPLATE_WEIGHT] = $result->Kentekengewicht . "";
        $return[ResourceInterface::IMPORTED_CAR]        = $result->Importauto . "";
        $return[ResourceInterface::LICENSEPLATE_COLOR]  = $result->Kentekenkleur . "";
        ...
        $return[ResourceInterface::AMOUNT_OF_DOORS]     = $result->Aantaldeuren . "";
        ...
        $return[ResourceInterface::ENERGY_LABEL]        = $result->Energielabel . "";;
        $return[ResourceInterface::TOP_SPEED]    = $result->Topsnelheid . "";
        $return[ResourceInterface::ACCELERATION] = $result->Acceleratie . "";

        $return[ResourceInterface::LABEL] = "{$result->Naam} ({$result->Aantaldeuren} deurs, {$return[ResourceInterface::POWER]} PK)";
